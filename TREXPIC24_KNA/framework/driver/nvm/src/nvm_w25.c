...
#endif
#ifndef FLASH_CS2
#error "FLASH_CS2 is not defined. Please define in system_config.h."
#endif
...
#define SPI_C_CMD_SER 0x20           // SECTOR ERASE
...
static inline __attribute__( ( __always_inline__ ) ) uint8_t _WriteSR( uint8_t cmd )
{
    uint8_t temp;
 ...
 *
 * @return boolean true if correct device is detected
 */
uint8_t EXTFLASH_JEDEC_Valid( void )
{
    uint32_t Response = 0;
    ...
    return ( Response == SPI_C_ID_W25Q128JV );
}
...
bool EXTFLASH_WritePage( uint32_t address, uint8_t *p )
{
  ...
  for(i = 0; i < 5; i++)
  {
    EXTFLASH_Read(address,&read_verify,1);
    if(!(read_verify == *data))
    {return false;}
    address++;
    data++;
...
uint8_t SectorErase(uint32_t address)
{
    ...
    // ERASE CHIP
    _EnableChipSelect();
    DRV_SPI_Put( SPI_CHANNEL_EXTFLASH, SPI_C_CMD_SER);
    DRV_SPI_Put( SPI_CHANNEL_EXTFLASH, ( address >> 16 ) & 0xff );
    DRV_SPI_Put( SPI_CHANNEL_EXTFLASH, ( address >> 8 ) & 0xff );
    DRV_SPI_Put( SPI_CHANNEL_EXTFLASH, ( address ) & 0xff );
