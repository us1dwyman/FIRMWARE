...
/* source address */
#if defined( J1939CFG_SA )
uint8_t j1939_sa = J1939CFG_SA;
#else
// should be assigned before J1939 initialization
uint8_t j1939_sa;
#endif
...
uint8_t j1939_tx_address_claimed( uint8_t src )
{
    ...
    // random start up delay to stop J1939 address claiming from colliding when multiple
    // devices power on at the same time.
    J1939_App_Delay();
...
void j1939_update_address_claim( void )
{
    ...
    if( j1939_address_claim_has_been_sent )
    {
        ...
        {
            // only notify application on transition from no address to address
            if( !j1939_address_claim_was_successful )
            {
                J1939_App_Address_Claimed( j1939_sa );
            }
...
void J1939_CheckRx( void )
{
    ...
    while( CAN_Rx( &can ) == 0 )
    {
        if(( can.id & CAN_EFF_FLAG ) == 0 )
        {
            J1939_AppProcessStd( &can );
        }
        else if( ( can.id & B25 ) == 0 )
        {
           // only process if EDP is not,
           j1939_post( &can ); 
        }
